...
static int ProcMsgVerOther(MsgHdr hdr, char *msg_str)
{
    ...
    if(msg_no == MSG_TEST_START)
    {
        if( g_ppTestMng[port]->IsTestOn(version) == 0 )
        {
            ...
            if(ssArg.str().empty())
            {
                SendMsg(idMsgq, version, cell, port, MSG_DONE,	0, "");
                SendMsg(idMsgq, version, cell, port, MSG_TEXT1,	0, "STRING EMPTY");
                return -2;
            }
            ...
            for(int i=0; i<vectArg.size(); i++)
            {
                if(i==0)
                {
                    strScriptPathName = vectArg.at(i);
                }
                else
                {
            ...
            if(!IsFileExist(strScriptPathName.c_str()))
            {
                ...
                ssExecScriptName << SYS_PATH << "/" << strScriptPathName;
                strScriptPathName = ssExecScriptName.str();

                if(!IsFileExist(strScriptPathName.c_str()))
                {
                    printf(">> port %d script is not exist!!\n", port);
                    SendMsg(idMsgq, version, cell, port, MSG_DONE,	0, "");
                    SendMsg(idMsgq, version, cell, port, MSG_TEXT1,	0, "SCRIPT NOT EXIST");
            ...
            else
            {
                ...
                string strPath;
                strPath.append(SYS_PATH);
                strPath.append("/");
                strPath.append(strScriptPathName.substr(0,posSplit));
                ...
                if(g_ppTestMng[port]->StartTest(version, strPath, strScriptName, strScriptArg) == 0)
                {
                    ...
                    SendMsg(idMsgq, version, cell, port, MSG_TEST,	0, "");
                    SendMsg(idMsgq, version, cell, port, MSG_TEXT1,	0, "TEST");
                }
            }
        }
        else
    ...
    else if(msg_no == MSG_TEST_STOP)
    {
        if(g_ppTestMng[port]->IsTestOn(version) == 1)
        {
            ...
            {
                SendMsg(idMsgq, version, cell, port, MSG_FAIL,	0, "");
                SendMsg(idMsgq, version, cell, port, MSG_TEXT1,	0, "ABORT OK");
            }
            else
            {
                SendMsg(idMsgq, version, cell, port, MSG_FAIL,	0, "");
                SendMsg(idMsgq, version, cell, port, MSG_TEXT1,	0, "ABORT FAIL");
            }
        }
    }
...
static int ProcMsgVerTest(MsgHdr hdr, char *msg_str)
{
    ...
    else if(msg_no == MSG_INIT)
    {
        ...
        case SC_FILE_DEFAULT:
            SendMsg(idMsgq, version, cell, port, MSG_INITACK,   SC_FILE_DEFAULT, szProgVersion);
            SendMsg(idMsgq, version, cell, port, MSG_TEXT1,    0,	"MSG_INIT");
            SendMsg(idMsgq, version, cell, port, MSG_TEXT2,    0, "");
            SendMsg(idMsgq, version, cell, port, MSG_TEXT3,    0, "");
            SendMsg(idMsgq, version, cell, port, MSG_TEXT4,    0, "");
            break;
        case SC_FILE_PASS:
            SendMsg(idMsgq, version, cell, port, MSG_INITACK,	SC_FILE_PASS, szProgVersion);
            SendMsg(idMsgq, version, cell, port, MSG_TEXT1,     0, "MSG_INIT");
            break;
        case SC_FILE_SIZE_ERR:
            SendMsg(idMsgq, version, cell, port, MSG_INITACK,	SC_FILE_SIZE_ERR, szProgVersion);
            SendMsg(idMsgq, version, cell, port, MSG_TEXT1,     0, "MSG_INIT");
            break;
        case SC_FILE_NONE_ERR:
            SendMsg(idMsgq, version, cell, port, MSG_INITACK,	SC_FILE_NONE_ERR, szProgVersion);
            SendMsg(idMsgq, version, cell, port, MSG_TEXT1,     0, "MSG_INIT");
            break;
